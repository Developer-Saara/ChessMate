...
class GameManager{
        ...
        setInterval(() => {
            this.#games.forEach((game, index) => {
                if (game.hasExceededTimeLimit()) {
                    console.log(`Game between ${game.player1Id} and ${game.player2Id} has exceeded 10 minutes`);
    ...
    addUsers(socket,userId){
        const game = this.#games.find(game => game.player1Id === userId || game.player2Id === userId )
        if (game) {
